...
directCostsByYear <- with(totalCosts, aggregate(directCost, by=list(Fin.Year), FUN=safeSum))
normalisedDirectCostsByYear <- with(totalCosts, aggregate(normalisedDirectCost, by=list(Fin.Year), FUN=safeSum))

# Plot a basic graph of costs
pdf(file="./figs/direct_costs.pdf")

# Calculate range from 0 to max value of costs
c_range <- range(0, directCostsByYear, normalisedDirectCostsByYear + 2000000000)

y_range <- range(totalCosts$Fin.Year)


plot(normalisedDirectCostsByYear, type="o", col="blue", ylim=c_range, axes=FALSE, ann=FALSE) 
...
# the actual plots 
legend(x="topleft", y=5000000000, legend=c("Normalised", "Original"), col=c("blue", "red"),
  lty=c(1,1), # gives the legend appropriate symbols (lines)
  lwd=c(2.5,2.5)) # gives the legend lines the correct color and width

# Add title
title("Direct Costs", col.main = "blue")
